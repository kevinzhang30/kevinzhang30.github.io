...
export default function BackToDashboard({ accent }: BackToDashboardProps) {
  ...
  return (
    <Link
      to="/"
      className="fixed top-6 left-6 z-30 flex items-center gap-2 rounded-full border bg-gray-950/80 px-4 py-2.5 font-mono text-xs uppercase tracking-[0.2em] backdrop-blur-md transition-all hover:bg-gray-900/90"
      style={{
        color: resolved,
        borderColor: `${resolved}55`,
        boxShadow: `0 0 14px ${resolved}25`,
      }}
    >
      <svg
        className="h-4 w-4"
