...
export default function RocketNode({
  target,
  active,
  focused,
  transitioning,
  ...
  useFrame((state) => {
    ...
    if (groupRef.current) {
      ...
      groupRef.current.position.y = target.position[1] + Math.sin(t * 1.2) * 0.32;

      // Rhythmic scale pulse when hovered
      const baseScale = focused ? (transitioning ? 1.32 : 1.14) : active ? 1.05 : 1;
      ...
      groupRef.current.position.z = THREE.MathUtils.lerp(
        ...
        target.position[2] + (focused && transitioning ? 2.4 : 0),
        0.1,
      );
    ...
    if (engineLightRef.current) {
      engineLightRef.current.intensity = focused
        ? transitioning
          ? 16
          : 12
        : active
      ...
      >
        ...
        <mesh position={[0, 1.78, 0.62]} rotation={[0.18, 0, 0]}>
          ...
          <meshStandardMaterial
            color="#091726"
            emissive="#75d1ff"
            emissiveIntensity={active || focused ? 0.8 : 0.4}
            roughness={0.08}
            metalness={0.55}
          />
        </mesh>
        ...
        <mesh position={[0, -2.25, 0]} scale={[1, 1.8, 1]}>
          ...
          <meshBasicMaterial color="#58b7ff" transparent opacity={focused ? 0.95 : 0.75} />
        </mesh>
